...
export default {
  ...
  methods: {
    ...
    setupRoutes() {
	  ...
	  this.$router.on("/index", () => {
        this.showPage("IndexPage", "Index Page");
      });
      this.$router.on("/news", () => {
        this.showPage("NewsPage", "Twitter News");
      });
      this.$router.on("/trade", () => {
        this.showPage("TradePage", "Trade Bot");
      });
      // modal routes
      this.$router.on("/history", () => {
        this.showModal("HistoryPage", "Recent Alert History");
      });
      this.$router.on("/alarms", () => {
        this.showModal("AlarmsList", "Saved Price Alarms");
      });
      this.$router.on("/about", () => {
        this.showModal("AboutPage", "About This App");
      });
      this.$router.on("/options", () => {
    ...
    onTickerError(e) {
      let info = String(
        e.message ||
          "Price ticker API connection failed, check the console for more details."
      );
      this.tickerStatus = 0;
    ...
    handleClick() {
      ...
      if (action === "binance") {
        ...
        let base =
          "https://www.binance.com" + dest + symb + "ref=" + this.refid;
